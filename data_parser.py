...
def parse_data(adress="Data\\RussianSamples.Mutations.UpTo28thFeb.txt"):
    ...
    parsed_data = parsed_data.rename(columns={0: 'sample name', 1: 'date', 2: 'location', 3: 'mutations'})

    for region_index in range(len(parsed_data['location'])):
        region = translate_regions_names_from_english_to_russian(parsed_data['location'][region_index])
...
def parse_regions(adress='Data\\RegionsInGADM.txt') :
...
file_for_data = open("Data\\data.csv", 'w')
file_for_regions = open("Data\\regions.txt", 'w')
file_for_mutations_names = open("Data\\mutations_names.txt", 'w')
...
for mutation_name_index in range(len(mutations_names)):
    if mutation_name_index != len(mutations_names) - 1:
        print(f"{mutations_names[mutation_name_index]}", file=file_for_mutations_names, end=',')
    else:
        print(f"{mutations_names[mutation_name_index]}", file=file_for_mutations_names, end='')

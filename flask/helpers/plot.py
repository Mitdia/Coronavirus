...
def create_plot(db, lang, min_date, max_date, width):
    ...
    buf_min_date = datetime.strptime(min_date, '%Y-%m-%d').strftime("%Y-%m")
    buf_max_date = datetime.strptime(max_date, '%Y-%m-%d').strftime("%Y-%m")
    datelist = pd.date_range(buf_min_date, buf_max_date, freq="MS").strftime("%Y-%m").tolist()
